...
电脑上的 NapCat 负责保持真实 QQ 在线，Gateway Class 把好友、群组和消息翻译成老客户端认识的格式。手机连接的是网关所在的那台机器，登录时不需要把真实 QQ 密码交给本项目。

默认形态是手机、网关和 NapCat 在同一个局域网。也支持把网关放到有公网 IP 的服务器上、NapCat 留在家里的电脑上（网关上云 + NapCat 留家里），完整步骤见包内的 [异地部署说明](nyanya-gateway-class/异地部署.md)。**这不是推荐形态**：老 QQ 协议是明文且无鉴权的，必须自行补公网防护。
...
> [零基础教程：用 J2ME / Symbian 老手机收发 QQ 消息](docs/零基础教程.md)

教程会从安装软件开始，逐步带你完成 NapCat、网关、J2ME JAR 或 Symbian Wi-Fi 设置，并告诉你每一步看到什么才算成功。

已经部署过的用户可以查阅 [使用与配置手册](docs/使用与配置手册.md)。
...
1. 双击 `nyanya-gateway-class/制作客户端.bat`；
...
要让手机连公网服务器时，双击 `制作客户端.bat` 后选 `2`（公网模式）输入服务器地址即可；也可直调 `nyanya-gateway-class/tools/patch-client.ps1` 并用 `-ServerAddress` 显式指定公网地址作为脚本化用法，详见包内的异地部署说明。

注入器会按客户端结构选择配置：两种已知 MobileQQ 12.0.16 布局使用完整补丁；其他包含 TCP 14000 入口的版本生成“实验性 TCP 核心版”，只重定向 TCP 并阻断剩余公网地址。看到 `experimental-tcp` 不代表旧版本协议已经兼容，仍需根据网关日志和真机逐项验证。已实测可直接使用的一例是 QQ2011 Beta1 Build 0012 未签名版（439 个 class，走实验性 TCP 核心版），登录、好友/群同步和群聊图片在真机上均已验证。带签名的 JAR 会被拒绝，因为修改会使原签名失效。
...
脚本会自动检测电脑 IP，不要求在电脑上输入手机 IP，但 Symbian 端必须先配置固定 IPv4 和子网掩码，默认网关才能正确生效。使用结束后，在脚本窗口按 Enter 恢复电脑设置，并把手机 IPv4、网关和 DNS 恢复原值或自动获取。完整填写示例见 [零基础教程](docs/零基础教程.md)。
...
图片收发已可用：群聊里的图片以原生气泡（可点击的「[图片]」）呈现，点开即可查看群图片；私聊图片推一条 WAP 链接；手机在群聊里发出的图片会经网关转发到真实 QQ。语音仍只会本地保存，不转发；搜索、加好友、群管理等操作没有完整映射；群消息不做离线补发。NapCat 和第三方 QQ 协议工具存在兼容及账号风控风险，不要使用主力账号；相关账号及数据损失由使用者自行承担，本项目及贡献者不承担责任。

本包不会包含你的 `config.json`、token、日志、PID、SQLite 数据、补丁 JAR 或逆向研究材料。请勿把 14000、13980、13981 或 NapCat 端口暴露到公网；确需异地访问时按包内的异地部署说明补齐防护。
