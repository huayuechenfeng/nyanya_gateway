...
如果你从未使用过 Node.js、NapCat，也不知道 JAR 注入和 Symbian Wi-Fi 网关是什么，请先按 [零基础 J2ME/Symbian 教程](零基础教程.md) 从头操作。那份教程只要求一台 Windows 电脑和一部能连接 Wi-Fi 的老手机。
...
| `loginPublicHost` | 空 | 登录响应报文里回给客户端的网关 IP。留空则自动探测局域网 IP；只接受 IPv4 点分字面量，填域名/IPv6 会被忽略并回退到自动探测。服务器部署（有 docker0 等虚拟网卡）时应写死公网 IP |
...
| 字段 | 默认值 | 用途 |
| --- | --- | --- |
| `friendPresence` | `10` | J2ME 路线不追踪好友真实在线状态，统一回给客户端的状态码（`10` 通常表示在线，`20` 表示离线） |
| `notifyIntervalSeconds` | `60` | 推给 J2ME 客户端的「新消息通知」检查间隔（秒），取值范围 5～86400；仅 J2ME 生效，Symbian 忽略 |
| `replayGroupHistoryOnLogin` | `false` | 开启后，客户端群接收就绪时把网关记录的最后若干条群消息当普通群消息补推，群窗口一打开就有内容 |
| `replayGroupHistoryLimit` | `10` | 每次回放的群消息条数 |
| `replayGroupHistoryDelayMs` | `1200` | 群回放延迟（毫秒）；客户端刚上报订阅清单时可能还在初始化群列表，太快推会被丢掉 |
| `replayPrivateHistoryOnLogin` | `false` | 开启后，客户端登录就绪时把网关记录的、**对方发来的**最后若干条私聊消息补推；私聊窗口一打开就有内容 |
| `replayPrivateHistoryLimit` | `10` | 每次回放的私聊消息条数 |
| `replayPrivateHistoryDelayMs` | `1500` | 私聊回放延迟（毫秒）；客户端登录后还要跑好友/群同步，太快推会被丢掉 |
| `replayCursorTtlMs` | `600000` | 回放水位有效期（毫秒）。水位记录每个会话「上次回放到哪条消息」，只推增量，避免掉线重连时重复推历史；超过该时间没动过就重新全量喂一遍。`0` 或负数表示永不过期 |

> **回放是「补历史」，代价是客户端会把它当新消息。** 老客户端不区分「客户端重启」和「掉线重连」，所以回放的消息在客户端看来就是新消息（未读、可能提示音），私聊还会写进本机记录。默认全关；只有确实希望「一登录就能看到最近的群/私聊内容」时再开。
...
Gateway Class 发布 ZIP 内置 `制作客户端.bat`，用户只需准备合法持有的 MobileQQ jar：

1. 让电脑先连接最终使用的局域网；
2. 双击 `nyanya-gateway-class/制作客户端.bat`；
...
**要连公网服务器（异地部署形态）时**：双击 `制作客户端.bat` 后，在地址模式里选 `2`（公网模式）并输入服务器 IP 或域名即可。直调 `tools/patch-client.ps1 -ServerAddress <公网IP或域名>` 作为脚本化/高级用法保留，详见下面的「异地部署」小节。
...
2. 手机连接与电脑互通的 Wi-Fi；
3. 双击 `nyanya-gateway-class/启用塞班路由.bat`，允许 UAC 管理员权限；
4. 记下脚本自动显示的电脑局域网 IP；
...
### 异地部署（网关上云 + NapCat 留家里）

如果手机经常不在家里、需要随时通过外网连上，可以把 Class 网关放到一台有公网 IP 的服务器上，NapCat 留在家里电脑，两者用 SSH 反向隧道（或 frp / WireGuard）相连。

**这不是项目推荐形态**：老 QQ 协议是明文且无鉴权的，把 14000 暴露到公网等于把 QQ 号挂在外网，必须自行补防护。这一路线的完整步骤、配置要点、systemd 服务和防火墙清单，见源码仓库及 Class 发布包内的 `nyanya-gateway-class/异地部署.md`。

和局域网形态的主要差异：

- `config.json` 要额外写死 `mediaPublicHost` 和 `loginPublicHost`（服务器上自动探测会挑到 `172.x` 内网地址）；
- `onebotUrl` 指向隧道入口（SSH 反向隧道下仍是 `ws://127.0.0.1:3001`）；
- 手机 JAR 用 `制作客户端.bat` 选公网模式（`2`）生成，或直调 `patch-client.ps1 -ServerAddress <公网IP>`；
- 家里那台电脑必须常开、隧道必须常连，否则手机登录后是空列表；
- `adminHost` 保持 `127.0.0.1`，管理页通过 SSH 端口转发访问；
- Symbian 路线（`启用塞班路由.bat`）在服务器形态下不可用，只支持 J2ME。
...
| 上云形态：手机能登录但好友/群为空 | 家里电脑是否开机、隧道窗口是否还在、网关日志有没有 `[napcat] 已连接` |
| 上云形态：手机收图打不开 | 检查启动日志里的「收图链接基址」是否为公网地址，必要时在 `mediaPublicHost` 写死 |
| 上云形态：手机连不上 | 安全组是否放行 14000/13981、JAR 是否用公网地址重新注入过 |
...
- 自己昵称、好友名、群名、群成员群名片；
- 私聊和群聊纯文字双向收发；
- 图片收发：群聊消息里的图片以原生气泡（可点击的「[图片]」）呈现，点开即可查看群图片；私聊图片推一条 WAP 链接供手机点开；手机在群聊里发出的图片经网关转发到真实 QQ；
- 私聊离线补发、系统通知；
- 登录后可将网关已有的群聊/私聊历史回放给客户端（默认关闭）；
...
- 群消息不做离线补发，手机离线期间的群消息只能在现代 QQ 补看；
- 私聊图片只能以 WAP 链接形式查看，不进聊天气泡（私聊窗口也没有发图入口）；
...
- 确需异地访问时，按 `nyanya-gateway-class/异地部署.md` 的形态部署，并补齐防火墙 / IP 白名单；老 QQ 协议是明文且登录摘要可重放，这层防护是唯一门锁；
- NapCat 和设备 token 使用不同值，不要复用真实 QQ 密码；公网形态下务必把 `deviceToken` 换成强口令；
