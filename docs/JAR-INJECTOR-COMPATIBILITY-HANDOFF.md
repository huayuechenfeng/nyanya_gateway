...
版本号只能作为辅助信息，不能作为唯一判断条件。同为 12.0.16 的两个样本已经证明其 class 数量和 HTTP 调用结构可能不同。

真机回归结果：两种 12.0.16、QQ2010 10.0.40 和 09.0.60 均可用；QQ2008 12.05.2 在登录阶段提示“付费用户余额不足（ID 31）”。该提示是当前网关响应与 QQ2008 登录解析不兼容的表现，不是实际账户余额结论。对外应推荐 QQ2009 及以上，QQ2008 及以下保留兼容性警告。
...
本项目已用上表五个本地样本完成端到端打包验证：两种 12.0.16 命中完整配置，另外三种命中实验性 TCP 配置。所有输出均通过 Manifest 首条目、JAD 大小和补丁后公网地址检查。真机测试进一步确认除 QQ2008 外的四个样本可用；QQ2008 12.05.2 登录失败并显示 ID 31。注入成功与协议兼容必须继续作为两个独立结论。
