...
- **水位（`core/replay-cursor.js`）**：每「账号+会话」记最后 `message.id` 只推增量；存内存不落库，带 `replayCursorTtlMs`。
  ...
  - **最终定案（2026-09-21 勇者拍板「不重复优先，私聊历史可有可无」）**：
    - 群：无本机落盘、重启需全量补历史 → 短 `replayCursorTtlMs`（120000），每次登录全量补推，聊天记录始终在。
    - 私聊：`ttlMs=0` **永不过期** → 网关重启后第一次登录全量推一次，之后每次登录只推增量（0 条）→ 私聊窗口历史为空（已接受）。
    - 取舍逻辑：私聊水位若也用短 TTL，客户端周期性 NAT 重连（间隔>>2min）会把含「加好友」文案的旧消息全量重推、周期弹通知；
      而私聊 RMS 又有「记录在、窗口渲染空白」老毛病（正文落 extra1、body 空），私聊历史其实也靠网关回放才显示 ⇒
      永不过期=私聊历史空窗。勇者选「不重复」放弃「私聊历史」。**新私聊消息仍实时推，不受水位影响。**
    - 教训：改回放水位 TTL 前先想清「群/私聊对重启容忍度相反」+「私聊 RMS 渲染空白 ⇒ 它其实也怕重启」，两者不能共用 TTL。
