...
Nyanya Gateway 是运行在电脑上的消息网关，通过与电脑上 NapCat 相连接并登录QQ号码，将QQ好友、群组和消息推送到终端，帮助终端上的客户端实现QQ聊天功能。
...
默认形态是手机、网关和 NapCat 在同一个局域网。Gateway Class 也支持把网关放到有公网 IP 的服务器上、NapCat 留在家里的电脑上，两者用隧道相连（见 [异地部署说明](nyanya-gateway-class/异地部署.md)）；这不是项目推荐形态，需要自行补公网防护。
...
> [零基础教程：从一台电脑和一部老手机开始](docs/零基础教程.md)
...
- 收发私聊文字消息和群聊文字消息；
- 图片收发：群聊里的图片以原生气泡呈现，点开即可查看群图片；私聊图片以 WAP 链接形式提供；手机在群聊里发出的图片会转发到真实 QQ；
- 群消息接收设置，以及网关侧的全局/按群屏蔽；
- 私聊离线补发、系统通知、本地管理页、运行日志和数据缓存。

语音目前只保存到本地、不转发，私聊图片不进聊天气泡；搜索、加好友、建群、邀请、踢人等管理操作也没有实现。详细边界见 [使用与配置手册](docs/使用与配置手册.md#功能范围与已知限制)。
...
- [零基础 J2ME/Symbian 教程](docs/零基础教程.md)：第一次部署从这里开始（局域网形态）。
- [使用与配置手册](docs/使用与配置手册.md)：配置字段、日常使用、升级和故障排查。
- [Gateway Class 子项目说明](nyanya-gateway-class/README.md)：旧 QQ 兼容层的能力与边界。
- [异地部署说明](nyanya-gateway-class/异地部署.md)：网关上云 + NapCat 留家里（公网形态）。
- [架构说明](架构说明.md)：适合希望理解代码分层的开发者。
- [Nyanya Protocol v1](docs/PROTOCOL.md)：第一方客户端开发者使用。
- [组件版本矩阵](docs/版本矩阵.md)：查看组件和协议兼容关系。
...
- 不要把 14000、13980、13981 或 NapCat 的端口直接映射到公网。确需异地访问时，按 [异地部署说明](nyanya-gateway-class/异地部署.md) 的形态部署并自行补齐防火墙 / IP 白名单等防护——老 QQ 协议是明文且可重放的，这层防护是唯一门锁。
- `deviceToken` 只是老手机连接网关的本地密码，不是 QQ 密码，但仍应使用独立值并妥善保存；公网形态下务必换成强口令。
